...
def gesture_recognition(lock, csi_amplitude_array, csi_shape, result_lock, action_array, people_array,
                         model_path="./model", task_action=6, task_people=8):
...
def gesture_recognition_plot(action_array, people_array, task_action=6, task_people=8):
    ...
    # ax1.set_xticklabels(action_name, rotation='vertical', fontsize=7, fontweight='bold')  # Display x-axis labels vertically
    ax2.set_ylabel("People")
    ax2.yaxis.set_label_position('right')
    bar_action = ax1.bar(action_name_short, action_arr)  # Create bar chart
    bar_people = ax2.bar(people_name, people_arr)        # Create bar chart

    while True:
        for bar, h in zip(bar_action, action_arr):
            bar.set_height(h)  # Update bar chart height
        ...
        plt.draw()
        plt.pause(0.3)  # Pause briefly to create dynamic animation effect
