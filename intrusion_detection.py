...
# Compute the cosine similarity of vectors A and B
def compute_cos(A, B):
    # Compute absolute values first (in case input is complex)
    A = np.abs(A)
    B = np.abs(B)
    return np.dot(A, B) / np.sqrt((np.dot(A, A) * np.dot(B, B)))


def intrusion_detection_func(lock, detection_lock, csi_amplitude_array, csi_shape, detection_data_array,
                              threshold=None, start_len=1000, detection_gap=1, alarm_interval=3000,
                              chosen_subcarrier=20, method=1, cache_len=100):
    set_threshold = False
    count = 0
    alarm_count = 0
    ...
    while True:
        ...
        if method == 0:  # Based on cosine similarity of CSI
            rate = 0.995
            ...
            if np.sum(cache_data[0]) != 0:
                if set_threshold and count < start_len:
                    ...
                    if count == start_len:
                        count += 1
                        print("Threshold set to:", threshold.value)
        ...
        elif method == 1:  # Based on amplitude range
            rate = 1.05
            if threshold.value == -1:
                threshold.value = 0.0
                set_threshold = True
        ...
        elif method == 2:  # Based on amplitude variance
            rate = 1.05
            if threshold.value == -1:
                threshold.value = 0.0
                set_threshold = True
...
def intrusion_history_func(host="10.20.14.42", user="zhaozijian", passwd="9213@fCOW",
                           db="wave_data", charset="utf8"):
...
def intrusion_plot(lock, detection_data_array, threshold, method=1, cache_len=100,
                   host="10.20.14.42", user="zhaozijian", passwd="9213@fCOW",
                   db="wave_data", charset="utf8", store_database=False):
